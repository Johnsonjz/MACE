...
class MACESOG(ScaleShiftMACE):
    def __init__(self, sog_arguments: Optional[Dict] = None, **kwargs):
        ...
        try:
            ...
            raise ImportError(
                "Cannot import 'sog'. Please install the 'sog' library from /data/zyjin/sog."
            ) from exc
    ...
    def forward(
        ...
        # Setting SOG cell input to zero when boundary conditions are not periodic
        cell_sog = cell.clone()
        pbc_tensor = data["pbc"].to(device=data["cell"].device)
        no_pbc_mask_cfg = ~pbc_tensor.any(dim=-1)
        no_pbc_mask_rows = no_pbc_mask_cfg.repeat_interleave(3)
